...
from pydantic import BaseModel, Field, validator
>>>>>>> 52da7d1b90f1c5ad71090c5faedde5b836d11a5d
...
class TicketInfo(BaseModel):
    validation= False
    uuid:str = Field(..., description = "name of the item", min_length=3,max_length=200, example= uuid.uuid4()) 
    date:str= Field(..., description = "creation date", example= datetime.utcnow())
...
=======
class TicketInfo(BaseModel):
    uuid: str = Field(..., description="name of the item",
                      min_length=3, max_length=36, example=uuid.uuid4())
    ...
    text: str = Field(..., description="Message  inside  the ticket",
                      min_length=5, max_length=50)
    level: int = Field(..., description="Title of the ticket",
                       min=1, max=5, example=3)
...
class UserInfoBase(BaseModel):
    username: str = Field(..., description="username of the website",
                          min_length=3, max_length=15, example="user_test")
    fullname: str = Field(..., description="username and last name",
                          max_length=20, example="John Doe")
    address: str = Field(..., description="address of the user")
    email: str = Field(..., description="user's email")
    date_of_brith: str = Field(..., description=" date of brith")
    max_open_ticket: int = Field(...,
                                 description="The maximmum ticket per user", max=5)
